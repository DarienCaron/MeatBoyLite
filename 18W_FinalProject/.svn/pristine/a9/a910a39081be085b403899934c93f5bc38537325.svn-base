...
ShaderProgram::ShaderProgram(const char* vertfilename, const char* fragfilename, const char* helperfile)
{
    m_VertShaderString = 0;
    m_FragShaderString = 0;
    m_StringsWereAllocated = false;
    ...
    Init( vertfilename, fragfilename, helperfile);
}
...
GLuint ShaderProgram::CompileShader(GLenum shaderType, const char* shaderstring, const char* helperfile)
{
    GLuint shaderhandle = glCreateShader( shaderType );

    const char* strings[] = { helperfile,shaderstring };
...
bool ShaderProgram::Init(const char* vertfilename, const char* fragfilename, const char* helperfile)
{
    char* vertstring = LoadCompleteFile( vertfilename, 0 );
    char* fragstring = LoadCompleteFile( fragfilename, 0 );
    char* helperstring = LoadCompleteFile(helperfile, 0);
...
bool ShaderProgram::InitFromStrings(char* vertstring, char* fragstring, char* helperfile)
{
    m_VertShaderString = vertstring;
    m_FragShaderString = fragstring;
    m_HelperString = helperfile;

    assert( m_VertShaderString != 0 && m_FragShaderString != 0);
    if( m_VertShaderString == 0 || m_FragShaderString == 0)
        return false;

    m_VertShader = CompileShader( GL_VERTEX_SHADER, m_VertShaderString, m_HelperString);
